...
download_file_and_asc() {
    ...
    # Check if the file exists and matches the checksum, if available
    if [ -f "$file_path" ]; then
        echo "$file_name already exists. Skipping download."
    else
        wget --continue "$url" -P "$output_dir"
    fi

    # Check if the .asc file exists
    local asc_file_path="$file_path.asc"
    if [ -f "$asc_file_path" ]; then
        echo "$file_name.asc already exists. Skipping download."
    else
        wget --continue "$url.asc" -P "$output_dir"
    fi
}
